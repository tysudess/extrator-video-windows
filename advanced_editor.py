...
class AdvancedVideoEditorWidget(QWidget):
    ...
    def _build_ui(self):
        ...
        title = QLabel('STUDIO // TIMELINE INTELIGENTE')
        title.setObjectName('EditorTitle')
        root.addWidget(title)
        desc = QLabel(
            'Portado do projeto Android v1.9.10: corte com tesourinha, exclusão de trechos, '
        ...
        self.btn_cut = QPushButton('✂ CORTAR NO CURSOR')
        ...
        self.zoom_label = QLabel('Zoom 4.8 px/s')
        ...
        clip_box = QGroupBox('PRECISÃO // CLIPE SELECIONADO')
        cb = QVBoxLayout(clip_box)
        self.selected_label = QLabel('Selecione um clipe na timeline')
        self.info_label = QLabel('')
        self.info_label.setWordWrap(True)
        cb.addWidget(self.selected_label); cb.addWidget(self.info_label)

        self.range = RangeSlider()
        cb.addWidget(self.range)
        self.local_position = QLabel('Posição: --:--')
        cb.addWidget(self.local_position)

        nudge = QHBoxLayout()
        nudge.addWidget(QLabel('Mover posição atual:'))
        self.nudge_buttons = []
        ...
        self.btn_apply = QPushButton('APLICAR TEMPOS')
        fields.addWidget(QLabel('Início:'), 0, 0); fields.addWidget(self.start_edit, 0, 1)
        fields.addWidget(QLabel('Fim:'), 1, 0); fields.addWidget(self.end_edit, 1, 1)
        fields.addWidget(self.btn_apply, 0, 2, 2, 1)
        cb.addLayout(fields)

        marks = QHBoxLayout()
        self.btn_mark_start = QPushButton('MARCAR INÍCIO AQUI')
        self.btn_mark_end = QPushButton('MARCAR FIM AQUI')
        marks.addWidget(self.btn_mark_start); marks.addWidget(self.btn_mark_end); marks.addStretch(1)
        cb.addLayout(marks)
    ...
    def _update_local_position(self, source_ms):
        clip = self._current_clip()
        if not clip:
            self.local_position.setText('Posição: --:--')
            return
        self.local_position.setText(f'Posição atual: {format_ms(source_ms)} / {format_ms(clip["duration_ms"])}')
